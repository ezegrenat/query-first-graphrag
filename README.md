...
El `.env` no se versiona: cada quien usa el suyo. `grafos/optimuskg/loader/config.py` lo lee al importarse, así que no hay que exportar nada a mano.

**3. Carga del grafo:**

```bash
python grafos/optimuskg/loader/load_neo4j.py
```
...
```bash
python grafos/optimuskg/preparacion_grafo/marcar_enfermedades_reales.py
```

Escribe la propiedad `is_truly_disease` en los nodos de la capa `Disease` y crea su indice. Acepta `--revertir` para deshacerlo.

El criterio y las mediciones que lo justifican están en `grafos/optimuskg/preparacion_grafo/limpieza_capa_disease.ipynb`, que se puede leer sin ejecutar porque conserva las salidas.
