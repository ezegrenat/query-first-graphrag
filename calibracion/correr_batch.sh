...
#(unas dos horas segun smoke.py), gene_a_disease corre en el plano de MONDO y tarda minutos.
#
...
[ -e "$DIR/casos.csv" ] || { echo "falta $DIR/casos.csv: correr paso0.py antes"; exit 1; }
echo "=== inicio $(date): 2 celdas, 50 por bin, presupuesto 100, sin corte"
$PY runner.py --celdas disease_a_gene > "$DIR/batch_disease_a_gene.log" 2>&1 &
$PY runner.py --celdas gene_a_disease > "$DIR/batch_gene_a_disease.log" 2>&1 &
sleep 5
./inhibir_suspension.sh > "$DIR/inhibidor.log" 2>&1 &
sleep 20
echo "procesos activos:"
ps -eo args | grep "^$PY runner.py" | sed 's/.*--celdas/  celdas:/'
